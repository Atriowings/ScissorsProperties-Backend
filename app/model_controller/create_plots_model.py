...
class PlotModel:
    ...
    def create_plot(self, plot):
        ...
        if plan_type == "A":
            ...
            paid_months = 1
            pending_months = 59
            pending_list = self.generate_month_list(1, total_months= 60) #total months 60 added
            next_due = 5000
            next_due_month = pending_list[0]
            next_due_date = now.replace(day=1) + timedelta(days=30)
            total_months = 60 #this one too added
            emi_size = 5000
        ...
        # new plan D added
            ...
            paid_months = 1
            pending_months = 149
            pending_list = self.generate_month_list(1, total_months=150)
            next_due = 2000
            next_due_month = pending_list[0]
            next_due_date = now.replace(day=1) + timedelta(days=30)
            total_months = 150
        ...
        base_plot_data = {
            ...
            # "totalMonths": 60, #total months 60 commented and add new line below
            "totalMonths": total_months, #new line added
            "emiSize": emi_size, #new line added
    ...
    def generate_month_list(self, start_index=0, total_months=60):
        now = datetime.utcnow().replace(day=1)
        return [(now + relativedelta(months=i)).strftime("%B %Y") for i in range(start_index, total_months)]
