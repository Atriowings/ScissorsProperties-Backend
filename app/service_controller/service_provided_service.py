from app.model_controller.service_provided_model import ServiceProvide
...
class ServiceProvideModel:
    ...
    def register_service(self,data):
        try:
            ...
            if self.service_provided_model.find_by_email(data.email):
                return None, "User already exists"

            service_id = self.service_provided_model.create_service(data)
    ...
    def service_login(self, email, password):
        service = self.service_provided_model.find_by_email(email)
        if service and self.service_provided_model.check_password(service['password'], password):
            return service, None
        if not service:
            return False, "User not found"
        if not self.service_provided_model.check_password(service['password'], password):
            return False, "Password is incorrect"

    def update_service_status(self, service_id, status):
        self.service_provided_model.update_status(service_id, status)


    def service_forgot_password(self, service_id, email):
        service = self.service_provided_model.find_by_service_id(service_id)
        if not service or service.get('email') != email:
            return None, "service not found or email mismatch"

        otp = generate_otp()
        self.service_provided_model.store_otp(service_id, otp)
        send_otp_email(email, otp)
        return otp, None

    def find_admin_by_email(self, email):
        return self.service_provided_model.find_by_email(email)

    def generate_otp(self, length=6):
        return ''.join(str(random.randint(0, 9)) for _ in range(length))

    def update_password(self, email, new_password):
        return self.service_provided_model.update_password(email, new_password)

    def find_user_by_otp(self, otp):
        return self.service_provided_model.find_by_otp(otp)
    ...
    def update_password(self, user_id, new_password):
        return self.service_provided_model.update_password(user_id, new_password)

    def store_otp(self, user_id, otp):
        return self.service_provided_model.store_otp(user_id, otp)

    def get_all(self, user_id):
        try:
            data = self.service_provided_model.get_user_and_payment(user_id)
            if not data['user']:
