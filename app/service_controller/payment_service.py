...
class PaymentService:
    ...
    def get_user_wallet_balance(self, user_id):
        ...
        if last_updated and (last_updated.month != now.month or last_updated.year != now.year):
            service_balance = 5000
            wallet_balance = service_balance + course_balance
            update_fields = {
                "collaboratorWallet.serviceWalletBalance": service_balance,
        ...
        return service_balance + course_balance, None
    ...
    def transfer_wallet_to_service(self, user_id, amount):
        ...
        if amount > service_balance:
            return None, "Insufficient service wallet balance"

        service_balance -= amount
        course_balance = wallet.get("courseWalletBalance", 0)
        wallet_balance = service_balance + course_balance
        ...
        update_fields = {
            "collaboratorWallet.serviceWalletBalance": service_balance,
            "collaboratorWallet.walletBalance": wallet_balance,
            "collaboratorWallet.amtTransferredToService": wallet.get("amtTransferredToService", 0) + amount
        }
    ...
    def transfer_wallet_to_course(self, user_id, amount):
        ...
        update_fields = {
            "collaboratorWallet.courseWalletBalance": course_balance,
            "collaboratorWallet.walletBalance": wallet_balance,
            "collaboratorWallet.amtTransferredToCourse": wallet.get("amtTransferredToCourse", 0) + amount
        }
    ...
    def request_money_addition(self, user_id, upi, upi_mobile_number, amount):
        ...
        update_fields = {
            ...
            "collaboratorWallet.requestedAddMoneyToCourseWallet": amount,
    ...
    def decline_wallet_addition(self, user_id):
        payment = self.payment_model.get_payment_by_user(user_id)
        if not payment:
            return False, "Payment record not found"
    ...
    def transfer_wallet_to_service(self, user_id, amount):
        return self.payment_model.transfer_course_to_service(user_id, amount)
